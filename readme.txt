...
= 1.8.2 =
* Added: on the monthly report, each listing in the per-listing breakdown now links to its public page, opening in a new tab so the report stays put. Suggested by the community. The report is usually opened from the email without being signed in, so the links go to the pages anyone can see rather than to the analytics tabs, which would have asked the reader to log in.
...
* Added: the report button in the email carries a signed link, so it opens on any device without signing in, and cannot be edited to reach another vendor's figures or another month. It stops working 90 days after the month it covers. The report is headed with the month it covers rather than a rolling "last 30 days".
* Fixed: the monthly email's list of figures ran together into a single paragraph in most email clients. Each figure is now on its own line.
* Added: on the monthly report, the up and down arrows compare the month against the whole of the previous month, and the report says which month that is. A month is never compared against a rolling window, which would have dropped a day whenever the two months were different lengths.
